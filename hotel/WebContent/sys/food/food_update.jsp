				...
				<div class="ItemBlock2">
                    ...
                    <tr>
                            ...
                            <select name="foodTypeId" style="width:80px">
			   					<c:forEach items="${requestScope.typeList}" var="foodType">
			   					 <c:choose>
			   					 	<c:when test="${requestScope.food.foodTypeId == foodType.id}">
			   							<option value="${foodType.id}" selected="selected">${foodType.foodTypeName }</option>
			   					 	</c:when>
			   					 	<c:otherwise>
						...
						<tr>
							...
							<td><input type="hidden" name="id" value="${food.id }"/>
								<input type="text" name="foodName" class="InputStyle" value="${food.foodName }"/> *</td>
						</tr>
						<tr>
							<td>价格</td>
